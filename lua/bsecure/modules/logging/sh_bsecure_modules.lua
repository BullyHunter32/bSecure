...
local KeyLogger = bSecure.Logs.CreateModule("Key Logs")
KeyLogger:Hook("bSecure.SuspiciousKeyPressed", function(pPlayer, strKey, strBind)
    KeyLogger:Log({
        ["Player"] = {
            "Name: "..pPlayer:Name(),
            "SteamID: "..pPlayer:SteamID64(),
        }
    },"%s has pressed a suspicious key. \"%s\" bound to \"%s\"", bSecure.FormatPlayer(pPlayer), strKey, strBind)
end)
