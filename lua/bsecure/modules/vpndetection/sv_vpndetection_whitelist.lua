...
hook.Add("bSecure.ShouldCheckVPN", "bSecure.Whitelist", function(pPlayer)
    if bSecure.VPN.Whitelists[pPlayer:SteamID64()] then
        return false
    end
end)

bSecure.VPN.AddWhitelist = function(SteamID, adminName, adminSteamID)
    if sql.Query(("SELECT * FROM bsecure_vpn_whitelist WHERE steamid64='%s'"):format(SteamID)) then bSecure.Print(SteamID .. " is already whitelisted!") return end
    sql.Query(("INSERT INTO bsecure_vpn_whitelist( steamid64, admin_name, admin_steamid ) VALUES( '%s', %s, '%s' )"):format(
        SteamID,
...
bSecure.VPN.RemoveWhitelist = function(SteamID, adminName, adminSteamID)
    sql.Query(("DELETE FROM bsecure_vpn_whitelist WHERE steamid64='%s'"):format(
        SteamID
    ))
...
bSecure.ConCommandAdd("addvpnwhitelist", function(pPlayer,CMD,tArgs)
    ...
    if not tArgs[2] or tArgs[2]:match("%a") or string.sub(string.lower(tArgs[2]),1,5) == "steam" then pPlayer:ChatPrint("You must provide a SteamID64!") return end
