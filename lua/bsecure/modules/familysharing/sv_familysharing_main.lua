...
hook.Add("PlayerInitialSpawn", "bSecure.CheckFamilyShare", function( pPlayer )
    ...
    if serverguard then
        serverguard_bans = sql.Query(("SELECT * FROM serverguard_bans WHERE community_id='%s' OR ip_address = '%s'"):format(
            OwnerSteamID,
            bSecure.FormatIP(pPlayer:IPAddress())
        ))
    end
    if ULib then
        ulib_bans = sql.Query(("SELECT * FROM ulib_bans WHERE steamid='%s'"):format(
            OwnerSteamID
        ))
    end
